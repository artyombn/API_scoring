...
@pytest.mark.parametrize(
    ...
    [
        ({"account": "horns&hoofs", "login": "h&f", "method": "online_score", "token": "", "arguments": {}}, api.FORBIDDEN, "Forbidden"),
        ({"account": "horns&hoofs", "login": "h&f", "method": "online_score", "token": "sdd", "arguments": {}}, api.FORBIDDEN, "Forbidden"),
        ({"account": "horns&hoofs", "login": "admin", "method": "online_score", "token": "", "arguments": {}}, api.FORBIDDEN, "Forbidden"),
    ]
)
...
@pytest.mark.parametrize(
    "request_body, code, response",
    [
        ({"account": "horns&hoofs", "login": "h&f", "method": "online_score"}, api.INVALID_REQUEST, "Invalid Request"),
        ({"account": "horns&hoofs", "login": "h&f", "arguments": {}}, api.INVALID_REQUEST, "Invalid Request"),
        ({"account": "horns&hoofs", "method": "online_score", "arguments": {}}, api.INVALID_REQUEST, "Invalid Request"),
    ]
)
def test_invalid_method_request(request_body, code, response, context, headers, settings):
    set_valid_auth(request_body)
    response_body, response_code = get_response(request_body, context, headers, settings)
    assert response_code == code
...
def test_invalid_score_request(arguments, context, headers, settings):
    ...
    assert code == api.INVALID_REQUEST, f"Expected {api.INVALID_REQUEST}, got {code} for arguments {arguments}"
